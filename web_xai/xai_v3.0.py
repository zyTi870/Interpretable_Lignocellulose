...
def add_scale_bar_2d(ax, image_width_px, pixel_size_um=1.32, bar_length_um=50):
    ...
    ax.text(x0 + bar_pixels/2, y0 - 10, f'{bar_length_um} $\mu m$',
            color='white', ha='center', va='bottom', fontsize=10, fontweight='bold', fontname='Nimbus Sans')
...
class UltimateXAIEngine:
    ...
    def _segmentation_3d_grid(self, image_input, block_size=(8, 16, 16)):
        ...
        # 广播机制生成 3D 索引
        # Z 维度需要乘以 (ny * nx), Y 维度乘以 (nx)
        ny = (H + bh - 1) // bh
        nx = (W + bw - 1) // bw

        # 利用 numpy broadcasting 构建 mask
    ...
    def run_lime(self, model, x, target_class, num_samples=50, num_features=5):
        ...
        使用 LIME 进行解释。
        参数:
        - num_samples: 扰动样本数量（越大越准但越慢，推荐 50-100）
        - num_features: 最终显示多少个最重要的 Superpixels
        """
        ...
        # 1. 数据格式转换
        # PyTorch: [1, C, D, H, W] -> LIME Numpy: [D, H, W, C]
        input_tensor = x.detach().cpu()
        input_np = input_tensor[0].numpy().transpose(1, 2, 3, 0) # (D, H, W, C)

        # 2. 定义预测函数 (Batch Predict)
        def batch_predict(images_np):
            # images_np: List of (D, H, W, C) -> Tensor (B, C, D, H, W)
            # 注意: lime 可能会传入 double，需要转 float32
        ...
        try:
            explanation = explainer.explain_instance(
                ...
                top_labels=None,
                hide_color=0,
                num_samples=num_samples, # 关键性能参数
                segmentation_fn=self._segmentation_3d_grid # 使用网格分割
            )
        except Exception as e:
            return None, f"LIME Error: {str(e)}"

        # 5. 提取 Mask
        # get_image_and_mask 返回 (image, mask)，我们只需要 mask
        # positive_only=True 表示只显示对预测有正向贡献的区域
        ...
        # Mask 是 bool 或 0/1 的 [D, H, W]，直接转为 float 用作 heatmap
        # LIME 结果是二值的（选中的块为 1，其他为 0）
...
def render_plotly_figure_v5(pts_data, title, camera_eye, volume_shape, is_preview=False, is_rg=False):
    if is_rg:
        ...
        marker_cfg = dict(size=2, color=color_strings, opacity=0.3, showscale=False)
    else:
    ...
    if not is_preview:
        ...
        offset = 10
        x_bar = [W - offset - bar_len_px, W - offset]; y_bar = [H - offset, H - offset]; z_bar = [0, 0]
        data.append(go.Scatter3d(
            x=x_bar, y=y_bar, z=z_bar, mode='lines+text',
            line=dict(color='black', width=3),
            text=[f"{scale_um} um", ""], textposition="top center",
            textfont=dict(color='black', size=14, family='Nimbus Sans'), showlegend=False
        ))
    axis_config = dict(
        showgrid=True, gridcolor='rgba(200, 200, 200, 0.5)',
        showline=True, linecolor='black', linewidth=2,
        backgroundcolor="rgba(255,255,255,0)", visible=not is_preview
    )
    layout = go.Layout(
        title=title, autosize=True, height=400 if is_preview else 600,
        scene=dict(xaxis=dict(title='X', **axis_config), yaxis=dict(title='Y', **axis_config), zaxis=dict(title='Z', **axis_config),
                   aspectmode='data', camera=dict(eye=camera_eye), dragmode='turntable'),
        margin=dict(l=0, r=0, b=0, t=30), paper_bgcolor="rgba(255, 255, 255, 0)"
    )
...
def save_rg_slice(vol_slice_2ch, hm_slice, title_str, save_name, hm_threshold=0.1):
    ...
    ax.axis('off')
    try: ax.set_title(title_str, fontname='Nimbus Sans', fontsize=14, pad=10)
