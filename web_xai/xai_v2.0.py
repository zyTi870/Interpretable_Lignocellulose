...
# 0. 基础配置
# ==========================================
st.set_page_config(layout="wide", page_title="CNN Fiber 3D XAI v5.4 (Fixed Missing Methods)")
...
class UltimateXAIEngine:
    ...
    # -----------------------------------------------------------
    # 修复部分：补全缺失的方法 (run_gradcam, run_cbam, run_vit_attn)
    # -----------------------------------------------------------
    ...
    def run_auto(self, model, x, method, arch_name, pred_class):
        ...
        if method == "Grad-CAM":
            ...
            res, _ = self.run_vit_attn(model, x)
            return res
        return None
